                ...
                {r.ingredients_raw && (
                  <div style={{ marginBottom:20 }}>
                    <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", marginBottom:10 }}>
                      ...
                      {(() => {
                        let items = null;
                        try { const p = JSON.parse(r.ingredients_raw); if (Array.isArray(p) && p[0]?.name) items = p; } catch {}
                    ...
                    <div style={{ background:"#fff", border:"1px solid var(--border)", borderRadius:12, overflow:"hidden" }}>
                      {(() => {
                        let items = null;
                        try { const parsed = JSON.parse(r.ingredients_raw); if (Array.isArray(parsed) && parsed[0]?.name) items = parsed; } catch {}
                        ...
                        if (items) {
                          ...
                          return <IngredientsList text={r.ingredients_raw} allergenFlags={rFlags} />;
                        }
                      })()}
                    </div>
                  </div>
