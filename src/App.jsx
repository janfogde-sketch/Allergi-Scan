...
// ─── Data ────────────────────────────────────────────────────────────────────
const ALLERGENS = [
  { id: "gluten",   label: "Gluten",          emoji: "🌾", desc: "Hvede, rug, byg, spelt" },
  { id: "laktose",  label: "Laktose / Mælk",  emoji: "🥛", desc: "Mælk, ost, smør, fløde" },
  { id: "aeg",      label: "Æg",              emoji: "🥚", desc: "Hele æg, æggehvide, æggeblomme" },
  { id: "noedder",  label: "Nødder",          emoji: "🥜", desc: "Jordnødder, mandler, valnødder mv." },
  { id: "soja",     label: "Soja",            emoji: "🫘", desc: "Sojabønner, sojamælk, tofu" },
  { id: "fisk",     label: "Fisk",            emoji: "🐟", desc: "Alle fiskearter" },
  { id: "skaldyr",  label: "Skaldyr",         emoji: "🦐", desc: "Rejer, krabber, muslinger" },
  { id: "selleri",  label: "Selleri",         emoji: "🥬", desc: "Selleri, selleriolie, sellerisalt" },
  { id: "sennep",   label: "Sennep",          emoji: "🌶️", desc: "Sennepsfrø, sennepspulver" },
  { id: "sesam",    label: "Sesam",           emoji: "🌿", desc: "Sesamfrø, tahini, sesamolie" },
  { id: "svovl",    label: "Sulfitter",       emoji: "🍷", desc: "Tørret frugt, vin, syltetøj" },
...
const MOCK_PRODUCTS = [
  { id:1, name:"Øko Havremælk", brand:"Oatly", emoji:"🥛", allergens:["gluten"], safe_for:["laktose"], stores:["netto","foetex","meny"], category:"Drikkevarer" },
  { id:2, name:"Glutenfri Brød", brand:"Schär", emoji:"🍞", allergens:["aeg"], safe_for:["gluten"], stores:["foetex","bilka","irma"], category:"Brød & bagværk" },
  { id:3, name:"Alpro Soja Drink", brand:"Alpro", emoji:"🫘", allergens:["soja"], safe_for:["laktose","gluten"], stores:["netto","meny","rema"], category:"Drikkevarer" },
  { id:4, name:"Lurpak Smør", brand:"Arla", emoji:"🧈", allergens:["laktose"], safe_for:["gluten"], stores:["netto","foetex","bilka","rema"], category:"Mejeri" },
  { id:5, name:"Røget Laks", brand:"Læsø Laks", emoji:"🐟", allergens:["fisk"], safe_for:["gluten","laktose"], stores:["netto","foetex"], category:"Fisk" },
  { id:6, name:"Cashewnødder", brand:"Sæson", emoji:"🥜", allergens:["noedder"], safe_for:["gluten","laktose"], stores:["netto","foetex","irma"], category:"Snacks" },
  { id:7, name:"Ris Naturel", brand:"Uncle Ben's", emoji:"🍚", allergens:[], safe_for:["gluten","laktose","noedder","soja","aeg"], stores:["netto","foetex","bilka","rema","aldi"], category:"Korn & ris" },
  { id:8, name:"Æble Juice",  brand:"Rynkeby",  emoji:"🍎", allergens:[], safe_for:["gluten","laktose","noedder"], stores:["netto","foetex","bilka","rema"], category:"Drikkevarer" },
  { id:9, name:"Kokosmælk", brand:"Aroy-D", emoji:"🥥", allergens:[], safe_for:["gluten","laktose","noedder","soja"], stores:["foetex","bilka","meny"], category:"Madlavning" },
  { id:10, name:"Havre Grød", brand:"Naturli", emoji:"🥣", allergens:["gluten"], safe_for:["laktose"], stores:["netto","foetex","rema","aldi"], category:"Morgenmad" },
];

const SCREENS = { ONBOARD:"onboard", HOME:"home", SCAN:"scan", SEARCH:"search", LIST:"list", PROFILE:"profile", FAMILY:"family", RESULT:"result" };
const AVATAR_COLORS = ["#52b788","#74c69d","#40916c","#b7e4c7","#2d6a4f","#95d5b2"];

// ─── Helpers ──────────────────────────────────────────────────────────────────
const uid = () => Math.random().toString(36).slice(2,9);
const initials = n => n.split(" ").filter(Boolean).map(w=>w[0]).join("").toUpperCase().slice(0,2)||"?";
const timeAgo = ts => { const d=Date.now()-ts; if(d<60000)return"lige nu"; if(d<3600000)return`${Math.floor(d/60000)} min siden`; if(d<86400000)return`${Math.floor(d/3600000)} t siden`; return`${Math.floor(d/86400000)} d siden`; };
...
html{height:-webkit-fill-available;}body{background:var(--bg);color:var(--text);font-family:var(--fb);min-height:100vh;min-height:-webkit-fill-available;}
.app{max-width:430px;margin:0 auto;min-height:100vh;display:flex;flex-direction:column;background:var(--bg);position:relative;width:100%;}
...
.screen{flex:1;padding:0 16px calc(90px + env(safe-area-inset-bottom));}
...
export default function AllergiScan() {
  const [screen,          setScreen]          = useState(() => { try { const u = localStorage.getItem("as_user"); return (u && JSON.parse(u)?.name) ? SCREENS.HOME : SCREENS.ONBOARD; } catch { return SCREENS.ONBOARD; } });
  const [onboardStep,     setOnboardStep]     = useState(1);
  const [editMode,        setEditMode]        = useState(false);
  const [user,            setUser]            = useState(load("as_user",       {name:"",age:"",email:"",phone:""}));
  ...
  const [activeProfiles,  setActiveProfiles]  = useState(load("as_active",     ["me"]));
  ...
  useEffect(()=>{ save("as_custom",customAllerg); },[customAllerg]);
  useEffect(()=>{ save("as_family",family); },[family]);
  useEffect(()=>{ save("as_active",activeProfiles); },[activeProfiles]);
  useEffect(()=>{ save("as_supermarkets",supermarkets); },[supermarkets]);
  useEffect(()=>{ save("as_history",history); },[history]);
  useEffect(()=>{ save("as_list",shoppingList); },[shoppingList]);
  

  const allActive = useCallback(() => {
    const ids = new Set(allergens);
    const cust = [...customAllerg];
    family.filter(m=>activeProfiles.includes(m.id)).forEach(m=>{ m.allergens.forEach(a=>ids.add(a)); m.custom.forEach(c=>{ if(!cust.includes(c)) cust.push(c); }); });
  ...
  const lookup = useCallback(async (code) => {
    ...
    try {
      ...
      if (data.status !== 1) { setScanError("Produktet blev ikke fundet. Prøv en anden stregkode."); setLoading(false); return; }
      ...
      const allergyList = [...aIds.map(id=>ALLERGENS.find(a=>a.id===id)?.label).filter(Boolean), ...aCust];
      const who = activeProfiles.length > 1 || (activeProfiles.length===1&&activeProfiles[0]!=="me")
        ? `Aktive profiler: ${[activeProfiles.includes("me")?user.name.split(" ")[0]:null,...family.filter(m=>activeProfiles.includes(m.id)).map(m=>m.name.split(" ")[0])].filter(Boolean).join(", ")}.` : "";
      const prompt = `Du er en præcis fødevareallergi-assistent. ${who} Allergier/intoleranser: ${allergyList.join(", ")||"ingen angivet"}.
Produkt: ${name}\nMærke: ${brand}\nIngredienser: ${ingredients||"(ikke tilgængeligt)"}\nAllergener fra database: ${allergenTags.join(", ")||"ingen"}
Analyser produktets sikkerhed. Svar KUN med JSON (ingen markdown): {"status":"safe"|"danger"|"warn","headline":"maks 7 ord på dansk","summary":"2 sætninger på dansk","flags":[{"type":"bad"|"maybe"|"good","text":"konkret observation på dansk"}]}`;
      const cr = await fetch("https://api.anthropic.com/v1/messages",{method:"POST",headers:{"Content-Type":"application/json"},
        body:JSON.stringify({model:"claude-sonnet-4-20250514",max_tokens:1000,messages:[{role:"user",content:prompt}]})});
      const cd = await cr.json();
      const txt = cd.content?.map(c=>c.text||"").join("")||"";
      const analysis = JSON.parse(txt.replace(/```json|```/g,"").trim());
      const newResult = {code, name, brand, ingredients, ...analysis, timestamp:Date.now()};
      setScanResult(newResult); setScreen(SCREENS.RESULT);
      setHistory(h=>[newResult,...h].slice(0,50));
    } catch { setScanError("Der opstod en fejl. Tjek din forbindelse og prøv igen."); }
    setLoading(false);
  ...
  const removeItem = (id) => setShoppingList(l=>l.filter(i=>i.id!==id));
  const clearDone  = ()   => setShoppingList(l=>l.filter(i=>!i.checked));

  const addMember = () => {
    if (!newMemberName.trim()) return;
    setFamily(f=>[...f,{id:uid(),name:newMemberName.trim(),allergens:newMemberAllerg,custom:[],color:AVATAR_COLORS[f.length%AVATAR_COLORS.length]}]);
    setNewMemberName(""); setNewMemberAllerg([]);
  };
  const removeMember = (id) => { setFamily(f=>f.filter(m=>m.id!==id)); setActiveProfiles(a=>a.filter(x=>x!==id)); };
  const toggleActive = (id) => setActiveProfiles(a=>a.includes(id)?a.filter(x=>x!==id):[...a,id]);
  const finishOnboard = () => { save("as_user",user); setScreen(SCREENS.HOME); setEditMode(false); };

  const {ids:activeIds} = allActive();
  const filteredProducts = MOCK_PRODUCTS.filter(p=>{
    const mQ = !searchQuery || p.name.toLowerCase().includes(searchQuery.toLowerCase()) || p.brand.toLowerCase().includes(searchQuery.toLowerCase());
    const mS = !searchStore || p.stores.includes(searchStore);
    return mQ && mS;
  }).map(p=>({...p, conflicts:activeIds.filter(id=>p.allergens.includes(id))}));
      ...
      <div className="app">
          ...
          <div className="onboard-wrap fade-in">
            {!editMode && (
              <div className="onboard-hero">
                ...
                <h1>Spis trygt med<br /><span>Allergi Scan</span></h1>
                <p>Din personlige allergiguide til danske supermarkeder. Opret din profil på 2 minutter.</p>
              </div>
            )}
            {editMode && <div style={{height:20}} />}
            <div className="step-dot-row">{[1,2,3,4].map(s=><div key={s} className={`step-dot${onboardStep===s?" active":""}`}/>)}</div>

            {/* Step 1 */}
            {onboardStep===1 && (
              <div className="card fade-in">
                <div className="step-title">👤 Fortæl os om dig</div>
                <div className="step-sub">Vi bruger disse oplysninger til din profil.</div>
                {[["Dit navn *","text","Fx. Anna Hansen","name"],["Alder","number","Fx. 32","age"],["E-mail","email","din@email.dk","email"],["Telefon","tel","+45 12 34 56 78","phone"]].map(([lbl,type,ph,key])=>(
                  <div key={key} style={{marginBottom:10}}>
                    <label className="field-lbl">{lbl}</label>
                    <input className="field" type={type} placeholder={ph} value={user[key]}
                      onChange={e=>setUser(u=>({...u,[key]:e.target.value}))} />
                  </div>
                ))}
                <button className="btn btn-primary btn-full" style={{marginTop:6}} onClick={()=>user.name.trim()&&setOnboardStep(2)}>Fortsæt →</button>
              </div>
            )}
            ...
            {onboardStep===2 && (
              <div className="fade-in">
                <div className="card">
                  ...
                  <div className="step-sub">Vælg de allergener du ikke kan tåle.</div>
                ...
                <div className="card">
                  ...
                  <div className="input-row" style={{marginBottom:customAllerg.length?10:0}}>
                    <input className="field" placeholder="Fx. Fructose, Histamin…" value={customInput} onChange={e=>setCustomInput(e.target.value)}
            ...
            {onboardStep===3 && (
              <div className="fade-in">
                <div className="card">
                  ...
                  {family.map(m=>(
                    <div key={m.id} className="family-member" style={{marginBottom:10}}>
                      <div style={{display:"flex",alignItems:"center",gap:10}}>
                        ...
                        <div style={{flex:1}}>
                          ...
                          <div style={{fontSize:11,color:"var(--muted)"}}>{m.allergens.map(id=>ALLERGENS.find(a=>a.id===id)?.emoji).join(" ")||"Ingen allergier"}</div>
                        </div>
            ...
            {onboardStep===4 && (
              <div className="fade-in">
                <div className="card">
                  ...
                  <div className="step-sub">Vælg de butikker du normalt handler i.</div>
                  <div className="sm-grid">
                    {SUPERMARKETS.map(s=>{ const on=supermarkets.includes(s.id); return (
                      <div key={s.id} className={`sm-chip${on?" on":""}`} onClick={()=>setSupermarkets(p=>on?p.filter(x=>x!==s.id):[...p,s.id])}>
                        <span className="sm-emoji">{s.emoji}</span><span>{s.label}</span>
                      </div>); })}
                  </div>
                </div>
                <div style={{display:"flex",gap:8}}>
                  <button className="btn btn-ghost btn-sm" onClick={()=>setOnboardStep(3)}>← Tilbage</button>
                  <button className="btn btn-primary" style={{flex:1}} onClick={finishOnboard}>{editMode?"Gem ændringer ✓":"Kom i gang →"}</button>
                </div>
        ...
        {screen===SCREENS.SCAN && (
          <div className="screen fade-in">
            ...
            <div className="screen-sub">Scan stregkoden på en madvare for at tjekke den mod dine allergier.</div>
            {family.length>0&&(
              <div style={{fontSize:12,color:"var(--g3)",fontWeight:700,marginBottom:12,padding:"7px 12px",background:"var(--g6)",borderRadius:9}}>
                🧑‍🤝‍🧑 Tjekker for: {[activeProfiles.includes("me")?user.name.split(" ")[0]:null,...family.filter(m=>activeProfiles.includes(m.id)).map(m=>m.name.split(" ")[0])].filter(Boolean).join(", ")}
              </div>)}
            <div className={`scanner-btn${scanning?" scanning":""}`} onClick={simulateScan}>
              {scanning ? (
                <><div className="scan-line-wrap">
                  <div className="scan-bars">{[24,36,50,62,50,36,24,50,62,36,24,50,36,62,24,50].map((h,i)=><div key={i} className="scan-bar" style={{height:h,animationDelay:`${i*.08}s`}}/>)}</div>
                  <div className="scan-beam"/>
                </div><div className="scan-title">Scanner stregkode…</div><div className="scan-sub">Hold produktet stille</div></>
              ) : (
                <><div className="scan-icon-big">📷</div><div className="scan-title">Tryk for at scanne</div><div className="scan-sub">Simulerer kamerascanning</div></>
              )}
            </div>
            ...
            {history.length>0&&!loading&&(
              <div className="card">
                <div className="card-lbl">Scanningshistorik ({history.length})</div>
                {history.slice(0,8).map((h,i)=>(
                  <div key={i} className="hist-row" onClick={()=>{setScanResult(h);setScreen(SCREENS.RESULT);}}>
                    <div className={`hist-dot ${h.status}`}/><div className="hist-info"><div className="hist-name">{h.name}</div><div className="hist-time">{timeAgo(h.timestamp)}</div></div>
                    <div className={`badge ${h.status}`}>{h.status==="safe"?"Sikker":h.status==="danger"?"Farlig":"Advarsel"}</div>
                  </div>))}
                <button className="btn btn-ghost btn-sm" style={{marginTop:10}} onClick={()=>{if(confirm("Ryd al historik?"))setHistory([]);}}>🗑 Ryd historik</button>
              </div>)}
          </div>
        )}
        ...
        {screen===SCREENS.RESULT&&scanResult&&(
          <div className="screen fade-in">
            ...
            <div className={`result-banner ${scanResult.status}`}>
              ...
              <div><div className={`rb-title ${scanResult.status}`}>{scanResult.headline}</div><div className="rb-sub">Stregkode: {scanResult.code}</div></div>
            </div>
            <div className="card">
              <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:14}}>
                ...
                <div><div style={{fontFamily:"var(--fh)",fontSize:16,fontWeight:800}}>{scanResult.name}</div>{scanResult.brand&&<div style={{fontSize:12,color:"var(--muted)",marginTop:2}}>{scanResult.brand}</div>}</div>
              </div>
              {scanResult.flags?.map((f,i)=><div key={i} className={`flag ${f.type}`}><span>{f.type==="bad"?"🚫":f.type==="maybe"?"⚠️":"✓"}</span>{f.text}</div>)}
              <div className="summary-txt" style={{marginTop:8}}>{scanResult.summary}</div>
              {scanResult.ingredients&&(
                <><div className="ing-toggle" onClick={()=>setShowIng(v=>!v)}><span>Ingrediensliste</span><span>{showIng?"▲":"▼"}</span></div>
                {showIng&&<div className="ing-text">{scanResult.ingredients}</div>}</>)}
            </div>
            <div className="card">
              <div className="card-lbl">Gem til indkøbsliste</div>
              <button className="btn btn-outline btn-full btn-sm" onClick={()=>{addToList(scanResult.name);setScreen(SCREENS.LIST);}}>🛒 Tilføj til indkøbsliste</button>
            </div>
          </div>
        )}
        ...
        {screen===SCREENS.SEARCH&&(
          <div className="screen fade-in">
            ...
            <div className="screen-sub">Find produkter der er sikre for dig i dine foretrukne supermarkeder.</div>
            <input className="field" placeholder="Søg på produkt eller mærke…" value={searchQuery} onChange={e=>setSearchQuery(e.target.value)} style={{marginBottom:12}}/>
            <div className="card-lbl">Filtrer på butik</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:6,marginBottom:14}}>
              <div className={`filter-chip${!searchStore?" active":""}`} onClick={()=>setSearchStore("")}>Alle butikker</div>
              {SUPERMARKETS.filter(s=>!supermarkets.length||supermarkets.includes(s.id)).map(s=>(
                <div key={s.id} className={`filter-chip${searchStore===s.id?" active":""}`} onClick={()=>setSearchStore(searchStore===s.id?"":s.id)}>{s.emoji} {s.label}</div>))}
            </div>
            {filteredProducts.length===0&&<div className="empty-list"><div className="empty-icon">🔍</div><div className="empty-txt">Ingen resultater</div><div className="empty-sub">Prøv en anden søgning</div></div>}
            {filteredProducts.map(p=>(
              <div key={p.id} className="product-card">
                ...
                <div style={{flex:1}}>
                  ...
                  <div className="product-stores">{p.stores.slice(0,3).map(id=>SUPERMARKETS.find(s=>s.id===id)?.label).filter(Boolean).join(", ")}</div>
                </div>
                <div style={{display:"flex",flexDirection:"column",alignItems:"flex-end",gap:5}}>
                  {p.conflicts.length>0?<div className="badge danger">⚠️ Konflikt</div>:<div className="badge safe">✓ Sikker</div>}
                  <button className="btn btn-ghost btn-sm" style={{fontSize:11,padding:"5px 9px"}} onClick={()=>addToList(p.name,p.stores[0])}>+ Liste</button>
                </div>
              </div>))}
            {filteredProducts.length>0&&<div style={{fontSize:12,color:"var(--muted)",textAlign:"center",padding:"8px 0"}}>{filteredProducts.filter(p=>p.conflicts.length===0).length} sikre produkter fundet</div>}
          </div>
        )}
        ...
        {screen===SCREENS.LIST&&(
          <div className="screen fade-in">
            ...
            <div className="screen-sub">Din personlige indkøbsliste. Del den med familien via link.</div>
            <div className="share-bar">
              <span style={{fontSize:18}}>🔗</span>
              <span className="share-txt">Del listen med familie via link</span>
              <button className="btn btn-ghost btn-sm" style={{fontSize:12}} onClick={()=>alert("Link kopieret! (Demo)")}>Kopiér</button>
            </div>
            <div className="card" style={{padding:"13px 14px",marginBottom:14}}>
              <div className="card-lbl">Tilføj vare</div>
              <div className="input-row" style={{marginBottom:8}}>
                <input className="field" placeholder="Fx. Glutenfri pasta…" value={newItemName} onChange={e=>setNewItemName(e.target.value)} onKeyDown={e=>e.key==="Enter"&&addToList(newItemName,newItemStore)}/>
                <button className="btn btn-primary btn-sm" style={{whiteSpace:"nowrap",width:"auto"}} onClick={()=>addToList(newItemName,newItemStore)}>Tilføj</button>
              </div>
              <select className="field" value={newItemStore} onChange={e=>setNewItemStore(e.target.value)} style={{fontSize:13}}>
                <option value="">Vælg butik (valgfrit)</option>
                {SUPERMARKETS.map(s=><option key={s.id} value={s.id}>{s.emoji} {s.label}</option>)}
              </select>
            </div>
            {shoppingList.length===0
              ? <div className="empty-list"><div className="empty-icon">🛒</div><div className="empty-txt">Indkøbslisten er tom</div><div className="empty-sub">Tilføj varer ovenfor</div></div>
              : <>
                  {shoppingList.filter(i=>!i.checked).length>0&&(
                    ...
                    {shoppingList.filter(i=>!i.checked).map(item=>(
                      <div key={item.id} className="list-item">
                        <div className={`list-check${item.checked?" checked":""}`} onClick={()=>toggleItem(item.id)}>{item.checked&&"✓"}</div>
                        <div style={{flex:1}}><div className="list-name">{item.name}</div>{item.store&&<div className="list-store">{SUPERMARKETS.find(s=>s.id===item.store)?.emoji} {SUPERMARKETS.find(s=>s.id===item.store)?.label}</div>}</div>
        ...
        {screen===SCREENS.PROFILE&&(
          <div className="screen fade-in">
            <div className="profile-hero">
              <div className="pa-lg" style={{background:"var(--g4)"}}>{initials(user.name)}</div>
              <div style={{flex:1}}><div className="profile-hero-name">{user.name}</div><div className="profile-hero-sub">{allergens.length+customAllerg.length} allergier · {family.length} familiemedlemmer</div></div>
            ...
            <div className="card">
              ...
              {[["📧","E-mail",user.email],["📱","Telefon",user.phone],["🎂","Alder",user.age]].map(([ic,lbl,val])=>val?(
                <div key={lbl} style={{display:"flex",gap:10,padding:"8px 0",borderBottom:"1px solid var(--border)",fontSize:13}}>
                  <span>{ic}</span><span style={{color:"var(--muted)",fontWeight:600,width:70}}>{lbl}</span><span style={{fontWeight:700}}>{val}</span>
                </div>):null)}
            </div>
            <div className="card">
              <div className="card-lbl" style={{display:"flex",justifyContent:"space-between"}}>
                <span>Mine allergier</span>
                <span style={{cursor:"pointer",color:"var(--g3)",fontWeight:700,fontSize:11}} onClick={()=>{setEditMode(true);setOnboardStep(2);}}>Rediger</span>
              </div>
              {allergens.length+customAllerg.length===0
                ? <div style={{fontSize:13,color:"var(--muted)"}}>Ingen registreret</div>
                : <div className="tags">{getAllergenLabels(allergens,customAllerg).map((a,i)=><div key={i} className="tag">{a}</div>)}</div>}
            </div>
            <div className="card">
              <div className="card-lbl" style={{display:"flex",justifyContent:"space-between"}}>
                <span>Mine supermarkeder</span>
                <span style={{cursor:"pointer",color:"var(--g3)",fontWeight:700,fontSize:11}} onClick={()=>{setEditMode(true);setOnboardStep(4);}}>Rediger</span>
              </div>
              {supermarkets.length===0
                ? <div style={{fontSize:13,color:"var(--muted)"}}>Ingen valgt</div>
                : <div className="tags">{supermarkets.map(id=>{const s=SUPERMARKETS.find(x=>x.id===id);return s?<div key={id} className="tag">{s.emoji} {s.label}</div>:null;})}</div>}
            </div>
            <div className="card">
              <div className="card-lbl">Scanningsstatistik</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:8,textAlign:"center"}}>
                {[[history.filter(h=>h.status==="safe").length,"✅","Sikre"],[history.filter(h=>h.status==="warn").length,"⚠️","Advarsler"],[history.filter(h=>h.status==="danger").length,"🚫","Farlige"]].map(([n,ic,lbl])=>(
                  <div key={lbl} style={{background:"var(--surface2)",borderRadius:11,padding:"12px 8px"}}>
                    <div style={{fontSize:20}}>{ic}</div><div style={{fontFamily:"var(--fh)",fontSize:22,fontWeight:900}}>{n}</div><div style={{fontSize:11,color:"var(--muted)",fontWeight:600}}>{lbl}</div>
                  </div>))}
              </div>
            </div>
            {history.length>0&&(
            ...
            <button className="btn btn-danger btn-full" style={{marginBottom:8}} onClick={()=>{if(confirm("Nulstil alt data og profil?")){ localStorage.clear(); window.location.reload(); }}}>Nulstil profil</button>
          </div>
        )}
        ...
        {screen===SCREENS.FAMILY&&(
          <div className="screen fade-in">
            ...
            <div className="screen-sub">Administrer familiemedlemmers allergiprofiler og filtrer for hele familien på én gang.</div>
            <div className="card" style={{padding:"12px 14px"}}>
              <div className="card-lbl">Aktive profiler (bruges ved scanning)</div>
              <div style={{display:"flex",flexWrap:"wrap",gap:7}}>
                <div className={`ap-chip${activeProfiles.includes("me")?" on":""}`} onClick={()=>toggleActive("me")}>
                  <div style={{width:20,height:20,borderRadius:"50%",background:"var(--g4)",display:"flex",alignItems:"center",justifyContent:"center",fontSize:10,fontWeight:800,color:"var(--g1)"}}>{initials(user.name)}</div>
                  {user.name.split(" ")[0]}
            ...
            {family.length===0&&<div className="empty-list" style={{padding:"30px 0"}}><div className="empty-icon">👨‍👩‍👧</div><div className="empty-txt">Ingen familiemedlemmer endnu</div><div className="empty-sub">Tilføj nedenfor</div></div>}
            {family.map(m=>(
              <div key={m.id} className="family-member">
                <div style={{display:"flex",alignItems:"center",gap:10,marginBottom:m.allergens.length?10:0}}>
                  <div className="fm-avatar" style={{background:m.color,color:"#fff"}}>{initials(m.name)}</div>
                  <div style={{flex:1}}><div style={{fontFamily:"var(--fh)",fontWeight:800,fontSize:15}}>{m.name}</div><div style={{fontSize:11,color:"var(--muted)",marginTop:2}}>{m.allergens.length+m.custom.length} allergier</div></div>
                  <span style={{cursor:"pointer",opacity:.4,fontSize:18,padding:4}} onClick={()=>removeMember(m.id)}>🗑</span>
                </div>
        ...
        {!isOnboard&&(
          <nav className="bottom-nav">
            {[[SCREENS.HOME,"🏠","Hjem"],[SCREENS.SCAN,"📷","Scan"],[SCREENS.SEARCH,"🔎","Søg"],[SCREENS.LIST,"🛒","Liste"],[SCREENS.FAMILY,"👨‍👩‍👧","Familie"]].map(([s,icon,lbl])=>(
              <div key={s} className={`nav-item${(screen===s||(screen===SCREENS.RESULT&&s===SCREENS.SCAN))?" active":""}`} onClick={()=>setScreen(s)}>
