        ...
        {filtered.map(([e, name], i, arr) => {
          ...
          return (
            <div key={e} className="enum-row"
              style={{ borderBottom: i < arr.length-1 ? "1px solid var(--border)" : "none", background: on?"var(--red-lt)":"var(--surface)" }}>
              <div onClick={() => onChange(on ? selected.filter(x=>x!==e) : [...selected, e])}
                style={{ display:"flex", alignItems:"center", gap:10, padding:"8px 12px", cursor:"pointer" }}>
                <div style={{ fontSize:12, fontWeight:800, color:on?"var(--red)":"var(--ink)", width:48, flexShrink:0 }}>{e}</div>
                <div style={{ fontSize:12, color:on?"var(--red)":"var(--ink2)", flex:1, lineHeight:1.4 }}>{shortName}</div>
                {detail && (
                ...
                {on && <div style={{ flexShrink:0 }}><Icon name="check" size={11} color="var(--red)" /></div>}
              </div>
              {isExpanded && detail && (
